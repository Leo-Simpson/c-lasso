import numpy as np
import matplotlib.pyplot as plt
import numpy.random as rd
import numpy.linalg as LA
from Functions.compact_func import Classo,pathlasso


def train_test_CV(n,k,test_pourcent):
    idx, training_size = rd.permutation(n), int(n-n*test_pourcent)
    idx_train, idx_test = idx[:training_size], idx[training_size:]
    SUBLIST,end = [],0 
    for i in range(k):
        begin,end = end,end+training_size//k
        if(i<training_size%k): end+=1
        SUBLIST.append(idx[begin:end])
    return(SUBLIST, idx_train,idx_test) 


def train_test_i (SUBLIST,i):
    training_set,test_set = np.array([],dtype=int),SUBLIST[i]
    for j in range(len(SUBLIST)):
        if (j != i): training_set = np.concatenate((training_set,SUBLIST[j]))
    return(training_set,test_set)
            

def training(matrices,typ,path, training_set):
    (A,C,y)   = matrices
    mat       = (A[training_set],C,y[training_set]) 
    return(pathlasso(mat,lambdas=path,typ=typ,meth='ODE',plot_time=False,plot_sol=False,plot_sigm=False,true_lam = True))


def test_i (matrices,typ,path, SUBLIST,i):
    training_set,test_set = train_test_i (SUBLIST,i)
    BETA,LAM              = training(matrices,typ,path, training_set)
    L = []
    for j in range(len(LAM)):
        L.append(accuracy_func(matrices[0][test_set],matrices[2][test_set],BETA[j],typ))
    return(L,LAM)

def average_test(matrices,typ,lamin, SUBLIST, lambdamax):
    n_lam = 100
    AVG = [0]*n_lam
    path=np.linspace(lambdamax,lambdamax*lamin,100)
    for i in range(len(SUBLIST)):
        L,LAM = test_i (matrices,typ,path, SUBLIST,i)
        for j in range(n_lam):
            AVG[j]+=L[j]
    return(AVG,LAM)

def CV(matrices,k,typ='LS',test=0.4,lamin=1e-2):
    (A,C,y) = matrices
    SUBLIST, idx_train, idx_test = train_test_CV(len(y),k,test)
    lambdamax = 2*LA.norm(matrices[0][idx_train].T.dot(matrices[2][idx_train]),np.infty)*(k-1)/k
    AVG, LAM = average_test(matrices,typ,lamin, SUBLIST, lambdamax)
    plt.plot(LAM,AVG),plt.xlabel("lambda"),plt.ylabel("||y-Ax||2"),plt.show()
    i = np.argmin(AVG)
    lam = LAM[i]/lambdamax
    print(lam)
    beta = Classo((A[idx_train],C,y[idx_train]),lam,typ=typ,plot_time=False,plot_sol=False,plot_sigm=False)
    return(beta)
    
        
# Cost fucntions for the three 'easiest' problems. 
def hub(r,rho) : 
    h=0
    for j in range(len(r)):
        if(abs(r[j])<rho): h+=r[j]**2
        elif(r[j]>0)     : h+= (2*r[j]-rho)*rho
        else             : h+= (-2*r[j]-rho)*rho
    return(h)

def accuracy_func(A,y,beta, typ='LS',rho = 1.345):      
    if (typ == 'Huber'):    return(hub( A.dot(beta) - y , rho * LA.norm(y,np.infty)/np.sqrt(len(y))) )
    else :                  return(LA.norm( A.dot(beta) - y )**2)