...
def test_random_data_C():
    ...
    for seed in range(10):
        ...
        mat, sol = random_data(n, d, d_nonzero, k, sigma, zerosum=False, A=np.eye(d), seed=seed, intercept=intercept)
        X, C, y = mat

        assert_allclose(C.dot(sol), 0., rtol=tol, atol=tol)
...
def test_random_data_zerosum():
    ...
    mat, sol = random_data(n, d, d_nonzero, k, sigma, zerosum=True, exp=True, classification=True, seed=None)
    X, C, y = mat

    assert set(y).issubset({1, -1})
    assert np.isclose(np.mean(sol), 0., atol=tol)
...
def test_random_data_C_big_k():
    ...
    mat, sol = random_data(n, d, d_nonzero, k, 0., zerosum=False, A=np.eye(d), seed=0)
    X, C, y = mat
...
def test_clr():
    X = np.eye(10)
    Z = clr(X)
    assert_allclose(np.mean(Z, axis=0), 0., rtol=tol, atol=tol)
